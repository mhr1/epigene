...
// A program to read the tandem duplicate list and the
...
int main(int argc, char** argv)
{
        ...
        // Open TD lst file
        sprintf(td_filename,"/dcs/mcr04/td_minus/TIGRs_TD_data.txt");
        ...
        while(fina.getline(buffer, 1024))
        {
            ...
            while(x_token)
            {
                if(j == 4) strcpy(gene_ID, x_token);

                x_token = strtok(NULL,"\t\n\r");
                j++;
            }

            //cout << "Found " << gene_ID << endl;

            if(((int)gene_ID[0] == 65) && j > 1) // A = 65 to detect A in At1g.....
            // Note we need j > 1 to detect if the line was blank
            // if line is blank gene_ID remains unchanged from last op
            // and will be counted again
            {
                    //cout << gene_ID << endl;
                    strcpy(td_gene_ID[i], gene_ID);
                    i++;
            }
            //
        ...
        cout << "Number of TDs is " << number_of_tds << endl;
        ...
        for(k = 1; k < 6; k++)
        {
            ...
            // Open output file
            sprintf(out_filename,"/dcs/mcr04/tigr_td_minus/C%d_%s.lox", k, file_ID);
